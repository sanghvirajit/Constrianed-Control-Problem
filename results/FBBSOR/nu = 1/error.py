...
yh = pd.read_csv("y_final_experimental_{}.txt".format(x), header=None)
y = pd.DataFrame(index=np.arange((yh.size)), columns=np.arange(1))

n = 15

for i in range(0, 129):
	y.iloc[i, 0] = y_df.iloc[i*31 + n, 0]
...
ph = pd.read_csv("p_final_experimental_{}.txt".format(x), header=None)
p = pd.DataFrame(index=np.arange((ph.size)), columns=np.arange(1))

for i in range(0, 129):
	p.iloc[i, 0] = p_df.iloc[i*31 + n, 0]
